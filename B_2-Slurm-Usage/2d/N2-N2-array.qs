...
# environment.
#
# Run 1444 separate jobs, indexed [1,1444], up to 10 executing
# concurrently:
#SBATCH --array=1-1444%10
#
...
# The job must have an array index provided by Slurm:
[ -n "$SLURM_ARRAY_TASK_ID" ] || exit 22

# Lookup the path to the given array index:
JOB_SUBDIR="$(job-catalog-lookup -ni $SLURM_ARRAY_TASK_ID -f _path_ job-mapping)"

# That path must exist and be a directory, of course:
cd "$JOB_SUBDIR" || exit 2
